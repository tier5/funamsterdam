...
class GFHANNANSMS_Pro_Verification
{
    ...
    public static function construct()
    {
        ...
        if (is_admin()) {
            add_filter('gform_add_field_buttons', array('GFHANNANSMS_Pro_Verification', 'gravity_sms_fields'));
            add_filter('gform_field_type_title', array('GFHANNANSMS_Pro_Verification', 'title'), 10, 2);
            add_action('gform_editor_js_set_default_values', array('GFHANNANSMS_Pro_Verification', 'default_label'));
            add_action('gform_editor_js', array('GFHANNANSMS_Pro_Verification', 'js'));
            add_action('gform_field_standard_settings', array('GFHANNANSMS_Pro_Verification', 'standard_settings'), 10, 2);
            add_filter('gform_tooltips', array('GFHANNANSMS_Pro_Verification', 'tooltips'));
        }

        add_filter('gform_field_validation', array('GFHANNANSMS_Pro_Verification', 'validation'), 10, 4);
        add_filter('gform_entry_post_save', array('GFHANNANSMS_Pro_Verification', 'process'), 10, 2);
        add_action('gform_field_input', array('GFHANNANSMS_Pro_Verification', 'input'), 10, 5);
        add_action('gform_field_css_class', array('GFHANNANSMS_Pro_Verification', 'classes'), 10, 3);
        add_filter('gform_field_content', array('GFHANNANSMS_Pro_Verification', 'content'), 10, 5);
        add_filter('gform_merge_tag_filter', array('GFHANNANSMS_Pro_Verification', 'all_fields'), 10, 4);
    }
            ...
            if ($is_form_editor) {
                ...
                if (!$diff_page && apply_filters('gform_button_verify', true) && empty($field_arr['conditionalLogic'])) {
                    ...
                    if (!empty($field_arr['pageNumber']) && $field_arr['pageNumber'] == $max_page_num || !empty($field_arr['pageNumber']))
                        add_filter('gform_submit_button', array('GFHANNANSMS_Pro_Verification', 'submit_button'), 10, 2);
                    else if ($max_page_num > 1)
                        add_filter('gform_next_button', array('GFHANNANSMS_Pro_Verification', 'next_button'), 10, 2);
                }
    ...
    public static function validation($result, $value, $form, $field)
    {
        ...
        if ($field_arr["type"] == "sms_verification") {
                    ...
                    if ($try_num <= $allowed_try && !rgempty('input_' . $field_arr["id"]) && !empty($code) && rgpost('input_' . str_replace( '.' , '_' , $field_arr["id"] ) ) == $code) {
                        ...
                        if ($try_num < $allowed_try) {
                            ...
                            if ($sent_num < $allowed_send) {
                                add_filter('sms_verify_resend', array('GFHANNANSMS_Pro_Verification', 'apply_true'), 99);
                            }
            ...
            if ($result["is_valid"] != true) {
                ...
                add_filter('gform_validation_message', array('GFHANNANSMS_Pro_Verification', 'change_message'), 10, 2);
                add_filter('sms_verify_display_none', array('GFHANNANSMS_Pro_Verification', 'apply_false'), 99);


                if ($show_input == true) {
                    add_filter('sms_verify_field', array('GFHANNANSMS_Pro_Verification', 'apply_true'), 99);
                }

                if (isset($result["action"]) && $result["action"] == 'self') {
                    $result["message_"] = $result["message"];
                } else {
                    add_filter('sms_verify_self_validation', array('GFHANNANSMS_Pro_Verification', 'apply_false'), 99);
                }
            } else {
                add_filter('gform_button_verify', array('GFHANNANSMS_Pro_Verification', 'apply_false'), 99);
            }
        ...
        foreach ((array)$sms_verification as $field) {
			...
			if (!empty($get_result) && is_object($get_result)) {
                ...
                if (!empty($ID) && $ID != 0 && !empty($status) && $status != 0) {
					...
					GFHANNANSMS_Pro_SQL::update_lead_verify_sent($form['id'], $entry['id'] , $verify_code);
    ...
    {
		$field = (array) $field;
        $code_type = rgget("sms_verify_country_code_radio", $field);
        if ($code_type == 'dynamic') {
            $code = rgget("field_sms_verify_country_code_dynamic", $field);
			$code = str_replace( '.' , '_' , $code);
            $code = "input_{$code}";
            $code = !rgempty($code) ? sanitize_text_field(rgpost($code)) : '';
        } else {
            $code = rgget("sms_verify_country_code_static", $field);
        }
        return $code;
    }
