...
class GFHANNANSMS_Pro_WP_SMS
{
    ...
    public static function construct()
    {
        ...
        add_filter('gform_add_field_buttons', array('GFHANNANSMS_Pro_WP_SMS', 'button'));

        if (!defined('WP_SMS_VERSION')) {
            add_filter('gform_admin_pre_render', array('GFHANNANSMS_Pro_WP_SMS', 'can_add'), 10, 4);
            return;
        }

        if (is_admin()) {
            add_filter('gform_field_type_title', array('GFHANNANSMS_Pro_WP_SMS', 'title'), 10, 2);
            add_filter('gform_editor_js_set_default_values', array('GFHANNANSMS_Pro_WP_SMS', 'default_label'));
            add_action('gform_editor_js', array('GFHANNANSMS_Pro_WP_SMS', 'js'));
            add_action('gform_field_standard_settings', array('GFHANNANSMS_Pro_WP_SMS', 'standard_settings'), 10, 2);
            add_filter('gform_field_content', array('GFHANNANSMS_Pro_WP_SMS', 'content'), 10, 5);
            add_filter('gform_entries_field_value', array('GFHANNANSMS_Pro_WP_SMS', 'entries_value'), 10, 4);
            add_filter('gform_tooltips', array('GFHANNANSMS_Pro_WP_SMS', 'tooltips'));
        }

        add_filter('gform_field_validation', array('GFHANNANSMS_Pro_WP_SMS', 'validation'), 10, 4);
        add_filter('gform_entry_post_save', array('GFHANNANSMS_Pro_WP_SMS', 'process'), 10, 2);
        add_filter('gform_merge_tag_filter', array('GFHANNANSMS_Pro_WP_SMS', 'merge_tag'), 10, 4);
        add_action('gform_field_input', array('GFHANNANSMS_Pro_WP_SMS', 'input'), 10, 5);
        add_action('gform_field_css_class', array('GFHANNANSMS_Pro_WP_SMS', 'classes'), 10, 3);
    }
        ...
        foreach ($field_groups as &$group) {
            if ($group["name"] == "gravity_sms_fields") {
                $group["fields"][] = array(
                    ...
                    "onclick" => "StartAddField('sms_subscribtion');"
                );
                break;
            }
        }
    ...
    public static function input($input, $field, $value, $lead_id, $form_id)
    {
        ...
        if ($field_arr["type"] == "sms_subscribtion") {
            ...
            if (!is_admin() && (rgget("wp_sms_subs", $field_arr) == 'unsubscribe' || rgget("wp_sms_group_select", $field_arr) == 'force')) {
                $hidden = '';
                if (!empty($field_arr['conditionalLogic'])) {
                    $hidden .= '<input name="input_' . $field_id . '" id="input_' . $form_id . '_' . $field_id . '" type="hidden" value="true" />';
                }
            ...
            if ($is_entry_page && rgget("wp_sms_subs", $field_arr) == 'unsubscribe') {
            ...
            $type = rgget("wp_sms_group_type", $field_arr) ? rgget("wp_sms_group_type", $field_arr) : 'radio';

            $desired_form_id = ((is_admin() && RG_CURRENT_VIEW != 'entry') || !is_admin()) ? $form_id . '_' : '';
            ...
            if ($is_form_editor) {
                ...
                if (rgget("wp_sms_subs", $field_arr) == 'unsubscribe') {
                    $style .= $hide_radio;
                    $style .= $hide_select;
                    $style .= $hide_force;
                } else {
                    $style .= $hide_unsubscribe;
                    if (rgget("wp_sms_group_select", $field_arr) != 'force') {
                        $style .= $hide_force;
                        if ($type == 'select') {
                            $style .= $hide_radio;
                        } else {
    ...
    public static function content($content, $field, $value, $lead_id, $form_id)
    {
        ...
        if (RG_CURRENT_VIEW == 'entry' && $mode == 'view' && $field["type"] == "sms_subscribtion") {
            $content = '<tr>
    ...
    public static function entries_value($value, $form_id, $field_id, $lead)
    {
    ...
    public static function country_code($field)
    {
		...
		$field = (array) $field;//
        $code_type = rgget("wp_sms_country_code_radio", $field);
        if ($code_type == 'dynamic') {
            $code = rgget("field_wp_sms_country_code_dynamic", $field);
			$code = str_replace( '.' , '_' , $code);
            $code = "input_{$code}";
            $code = !rgempty($code) ? sanitize_text_field(rgpost($code)) : '';
        } else {
            $code = rgget("wp_sms_country_code_static", $field);
        }
        return $code;
    }
    ...
    public static function validation($result, $value, $form, $field)
    {
        ...
        if ($field_arr["type"] == "sms_subscribtion") {
            ...
            if (rgget("wp_sms_subs", $field_arr) != 'unsubscribe' && rgget("wp_sms_repeat_error", $field_arr)) {

                $mobile = rgget("field_wp_sms_mobile", $field_arr);
				$mobile = str_replace( '.' , '_' , $mobile);
                $mobile = "input_{$mobile}";
                $mobile = !rgempty($mobile) ? sanitize_text_field(rgpost($mobile)) : '';
                $mobile = GFHANNANSMS_Form_Send::change_mobile_separately($mobile, self::country_code($field_arr));
                ...
                if ($mobile_exist) {
                    ...
                    $result["message"] = rgget("wp_sms_repeat_mgs", $field_arr) ? rgget("wp_sms_repeat_mgs", $field_arr) : __("The entered mobile number has previously been used in this form.", "GF_SMS");
                    add_filter('wp_sms_display_none', array('GFHANNANSMS_Pro_WP_SMS', 'apply_false'));
                }

            }

            if (RGFormsModel::get_input_type($field) == 'adminonly_hidden' || rgget("wp_sms_subs", $field_arr) == 'unsubscribe' || (rgget("wp_sms_group_select", $field_arr) == 'force' && !$mobile_error)) {
    ...
    public static function process($entry, $form)
    {
        ...
        foreach ((array)$wp_sms_fileds as $field) {
            ...
            $type = rgget("wp_sms_subs", $field) ? rgget("wp_sms_subs", $field) : 'subscribe';

            $name = rgget("field_wp_sms_name", $field);
			$name = str_replace( '.' , '_' , $name);
            $name = "input_{$name}";
            $name = !rgempty($name) ? sanitize_text_field(rgpost($name)) : '';

            $mobile = rgget("field_wp_sms_mobile", $field);
			$mobile = str_replace( '.' , '_' , $mobile);
            $mobile = "input_{$mobile}";
            $mobile = !rgempty($mobile) ? sanitize_text_field(rgpost($mobile)) : '';
            $mobile = GFHANNANSMS_Form_Send::change_mobile_separately($mobile, self::country_code($field));

            if (rgget("wp_sms_group_select", $field) == 'force') {
                $groups = rgget("wp_sms_group_forced", $field);
            } else {
            ...
            $process = self::subscribtion($name, $mobile, $groups, $type);
            ...
            if (!empty($process['status']) && $process['status'] == 'success-1') {
                ...
                if (get_option('wp_subscribes_send_sms') && rgget("wp_sms_welcome_msg", $field)) {
    ...
    public static function subscribtion($name, $mobile, $groups, $type)
    {
        ...
        if (!$mobile_exist || $type != 'subscribe') {
            ...
            if ($type == 'subscribe') {
                ...
                if ($mobile_exist) {
                    ...
                    $delete = $wpdb->delete("{$table_prefix}sms_subscribes", array('mobile' => $mobile));
