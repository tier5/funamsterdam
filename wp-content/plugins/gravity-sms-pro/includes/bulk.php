...
class GFHANNANSMS_Pro_Bulk
{
    ...
    {	
        ...
        add_filter('gform_entry_apply_button', array('GFHANNANSMS_Pro_Bulk', 'bulk_action_old'), 10, 1);
        add_action('admin_footer', array('GFHANNANSMS_Pro_Bulk', 'bulk_action_new'));
        add_action('admin_footer', array('GFHANNANSMS_Pro_Bulk', 'bulk_action'));
    }
    ...
    { ?>
        ...
        if ( rgpost('action') == 'sendsms' || rgpost('action2') == 'sendsms' || rgpost('bulk_action') == 'sendsms' || rgpost('bulk_action2') == 'sendsms' ) {
            ...
            $leads = rgpost("all_entries") ? GFFormsModel::search_lead_ids($form_id, '') : ( rgpost("entry") ?  rgpost("entry") : rgpost("lead") );
			
            $clients = array();
            foreach ((array)$leads as $lead_id) {
                $lead = RGFormsModel::get_lead($lead_id);
                $clients[] = GFHANNANSMS_Pro_Entries_Sidebar::get_phone_numbers($form_id, $lead);
            }
            ...
            if (!empty($clients)) { ?>
                <form id="bulk_confirmation" method="post" action="admin.php?page=gf_hannansms&view=send">
                    ...
                    <input type="hidden" name="hannansms_bulk_leads" value="<?php echo implode(',', $leads) ?>"/>
    ...
    public static function send_many_numbers()
    {
        ...
        if (rgpost("hannansms_bulk_numbers")) {
            ...
            $leads = rgpost("hannansms_bulk_leads") ? explode(',', sanitize_text_field(rgpost("hannansms_bulk_leads"))) : array();
        } else if (!rgempty("gf_hannansms_submit_send") && rgpost("gf_hannansms_form_id_send") && rgpost("gf_hannansms_leads_send")) {
            $form_id = rgpost("gf_hannansms_form_id_send") ? absint(rgpost("gf_hannansms_form_id_send")) : "";
            $leads = rgpost("gf_hannansms_leads_send") ? explode(',', sanitize_text_field(rgpost("gf_hannansms_leads_send"))) : array();
        }

        ?>
        <div class="wrap gforms_edit_form gf_browser_gecko">
		
            <img alt="<?php _e("HANNANSMS", "GF_SMS") ?>" style="margin: 15px 7px 0pt 0pt; float: left;"
            ...
            if (!rgempty("gf_hannansms_submit_send")) {
                ...
                if ($to) {
                    ...
                    if ($result == 'OK') {
                        ...
                        GFHANNANSMS_Pro_SQL::save_sms_sent($form_id, $leads, $from, $to, $msg , '');
                        echo '<div class="updated fade" style="padding:6px">' . __("Message sent successfully", "GF_SMS") . '</div>';

                        if (rgpost("gf_hannansms_form_id_send") && rgpost("gf_hannansms_leads_send")) {
                            foreach ((array)$leads as $lead_id) {
                                $lead = RGFormsModel::get_lead($lead_id);
                                $clients = GFHANNANSMS_Pro_Entries_Sidebar::get_phone_numbers($form_id, $lead);
                                if (!empty($clients)) {
                                    ...
                                    RGFormsModel::add_note($lead_id, $user_id, $user_name, $receiver_note);
                                }
                            }
                        }
                        ...
                        if (rgpost("gf_hannansms_form_id_send") && rgpost("gf_hannansms_leads_send")) {
                            foreach ((array)$leads as $lead_id) {
                                ...
                                if (!empty($clients)) {
                                    ...
                                    RGFormsModel::add_note($lead_id, $user_id, $user_name, $reciever_note);
                                }
                            }
                        }
            ...
            <div class="postbox" style="padding:0 30px 20px; margin-top:10px;background:#F6FBFD;">
				...
				<form method="post" action="">
                    ...
                    <?php if ($is_OK) {
                        ...
                        <?php if (!empty($form_id) && !empty($leads)) { ?>
                            <input type="hidden" name="gf_hannansms_form_id_send" value="<?php echo $form_id; ?>"/>
                            <input type="hidden" name="gf_hannansms_leads_send"
                                   value="<?php echo implode(',', $leads) ?>"/>
                        <?php }
