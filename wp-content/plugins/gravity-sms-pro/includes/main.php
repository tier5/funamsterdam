...
class GFHANNANSMS_Pro
{
    ...
    public static function construct()
    {
        ...
        if (function_exists('members_get_capabilities'))
            add_filter('members_get_capabilities', array('GFHANNANSMS_Pro', 'members_get_capabilities'));
        ...
        require_once(GF_SMS_DIR . 'includes/functions.php');
        ...
        if (is_admin()) {
			...
			if (!get_option('gf_sms_pro_notice_1')) {
				add_action('admin_notices', array('GFHANNANSMS_Pro', 'admin_notice'));
				add_action('wp_ajax_nopriv_dismiss_admin_notice_gf_sms', array('GFHANNANSMS_Pro', 'dismiss_admin_notice_gf_sms'));
				add_action('wp_ajax_dismiss_admin_notice_gf_sms', array('GFHANNANSMS_Pro', 'dismiss_admin_notice_gf_sms'));
			}
			

            add_action('admin_bar_menu', array('GFHANNANSMS_Pro', 'admin_bar_menu'), 2000);
            add_filter('gform_addon_navigation', array('GFHANNANSMS_Pro', 'submenu'));
    ...
    public static function submenu($submenus)
    {
        ...
        if (!empty($permission)) {
            $submenus[] = array(
                ...
                "callback" => array("GFHANNANSMS_Pro", "pages"),
    ...
    public static function admin_bar_menu()
    {
        ...
        global $wp_admin_bar;
        $menu_id = 'GF_SMS';
        $wp_admin_bar->add_menu(array('id' => $menu_id, 'title' => (__('GF SMS', 'GF_SMS') . $balance), 'href' => 'admin.php?page=gf_settings&subview=gf_sms_pro'));
        $wp_admin_bar->add_menu(array('parent' => $menu_id, 'title' => __('General Settings', 'GF_SMS'), 'id' => 'gf-sms-settings', 'href' => 'admin.php?page=gf_settings&subview=gf_sms_pro'));
        $wp_admin_bar->add_menu(array('parent' => $menu_id, 'title' => __('Feeds', 'GF_SMS'), 'id' => 'gf-sms-feeds', 'href' => 'admin.php?page=gf_hannansms'));
        $wp_admin_bar->add_menu(array('parent' => $menu_id, 'title' => __('New Feed', 'GF_SMS'), 'id' => 'gf-sms-new-feed', 'href' => 'admin.php?page=gf_hannansms&view=edit&id=0'));
        $wp_admin_bar->add_menu(array('parent' => $menu_id, 'title' => __('Send Message', 'GF_SMS'), 'id' => 'gf-sms-send', 'href' => 'admin.php?page=gf_hannansms&view=send'));
        $wp_admin_bar->add_menu(array('parent' => $menu_id, 'title' => __('Sent Messages', 'GF_SMS'), 'id' => 'gf-sms-sent', 'href' => 'admin.php?page=gf_hannansms&view=sent'));

        $feeds = GFHANNANSMS_Pro_SQL::get_feeds();
        if (is_array($feeds) && sizeof($feeds) > 0) {
            rsort($feeds);
            foreach ((array)$feeds as $feed) {
                $wp_admin_bar->add_menu(array('parent' => 'gf-sms-feeds', 'title' => __('Feed', 'GF_SMS') . ' ' . $feed['id'] . ' (' . $feed["form_title"] . ')', 'id' => 'gf-sms-feed-' . $feed['id'], 'href' => 'admin.php?page=gf_hannansms&view=edit&id=' . $feed['id']));
            }
        }
    ...
    {	
        ...
        $message = sprintf(__('Do you ❤️ "Gravity Forms SMS Pro"? Please Rate 5 Star on %sCodecanyoun%s.', 'GF_SMS') , '<a target="_blank" href="https://codecanyon.net/user/hannansoft/portfolio">' , '</a>');
    ...
    public static function dismiss_admin_notice_gf_sms()
    {
		...
		delete_option('gf_sms_pro_notice');
        update_option('gf_sms_pro_notice_1', 'true');
        die();
    }
